function BackupManager(config) {
    ...
    me.restore = function () {
		...
		'[ "$COMPUTE_TYPE" == "redis" ] && { RESTIC_PASSWORD=$(cat /root/.backupedenv) restic -r /opt/backup/$(cat /root/.backupedenv) restore ${SNAPSHOT_ID} --target /; } || true',
		'[ "$COMPUTE_TYPE" == "redis" ] && { rm -f /root/redis-restore.sh; wget -O /root/redis-restore.sh %(baseUrl)/scripts/redis-restore.sh; chmod +x /root/redis-restore.sh; bash /root/redis-restore.sh; } || true',
